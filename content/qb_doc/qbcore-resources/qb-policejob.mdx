...
* このリソースは警察の機能とやり取りを管理します
* 警察が設置する物品[qb-ラジアルメニュー.md](qb-radialmenu.md "mention")交通コーンやスパイクストリップなど
* 警察の隠し場所、ガレージ、武器庫
* 選手のカフ機能
* 血液DNA滴や指紋を含む証拠システム
* 警察が選手を追跡するためのアンクレットトラッカー
...
* playerが射撃しているときに弾丸の薬莢がランダムにドロップする確率
* playerが手袋を着用していない場合に指紋が残る確率
* 特定のアクションを実行するとプレイヤーのステータスが設定されます

### 構成

* qb-policejob/client/evidence.lua で見つかりました
...
<details>
  ...
  このコマンドは、ジョブ「警察」を持つすべてのプレイヤーにアラートを送信します。アラートには指定された内容が含まれます。`message`そして、あなたの現在位置の警察の player マップにマークが追加されます。
...
<details>
  ...
  地面にスパイクストリップオブジェクトを設置します。使用するには、player が「警察」の職務に就き、勤務中である必要があります。
...
<details>
  ...
  指定されたplayerを返す`id`与えられたライセンス`license` タイプ。
...
<details>
  ...
  「警察」ジョブを持つplayerがセキュリティカメラを監視できるようにします。セキュリティカメラの位置はqb-policejobの設定ファイルで設定できます。`Config.SecurityCameras`。
...
<details>
  ...
  '警察'の職務を持つplayerが、指定された期間内にvehicleを押収物として送ることを許可します。`price`
...
<details>
  ...
  与えられたplayerを支払う`id`500ドル。支払われるplayerは「牽引」の任務を担う必要があります。
...
<details>
  ...
  与えられたplayerを支払う`id`500ドル。報酬を受け取るplayerは「弁護士」の職業に就いている必要があります。
